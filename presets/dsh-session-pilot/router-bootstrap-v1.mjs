...
export const inject = ['systemPrompt', 'tools', 'llm', 'subagents']
...
export function apply(ctx, config) {
  ...
  function legacyCore(mode) {
    const smTools = [
      'session_list_models', 'session_set_model',
      'dev_mode_subagent',
      'prompt_set_system', 'prompt_append_system',
      'prompt_set_context', 'prompt_append_context',
      'prompt_clear', 'prompt_status',
    ]
    switch (bandOf(mode)) {
      case 'spec': return ['read', 'edit', 'glob', 'grep', ...smTools]
      default: return ['read', 'write', 'edit', ...smTools]
    }
  }
  ...
  registerTool({
    ...
    description: 'Run one task in a DIFFERENT reasoning mode than this session, in a fresh isolated context (own system prompt). The current session trajectory is untouched. Mode: spec (plan-first) / weak (internal routing) / react (doer) / balanced. Returns the subagent\'s answer text. Creates a real DSH subagent so it appears in the subagent catalog / task management.',
    parameters: {
      ...
      provider: { type: 'string', description: 'optional provider route; defaults to the current session model' },
      model: { type: 'string', description: 'optional exact model id; defaults to the current session model' },
    ...
    async execute(args, exec) {
      ...
      const agent = exec?.agent ?? currentAgent()
      if (agent === undefined || agent.options === undefined) return 'no agent route available'
      const logged = agent.session?.requestHeader?.()?.config
      let provider = args.provider ?? logged?.provider ?? agent.options.provider
      let model = args.model ?? logged?.model ?? agent.options.model
      if (Boolean(args.provider) !== Boolean(args.model)) {
        return 'error: provider and model must be provided together (or both omitted)'
      }
      if (!provider || !model) return 'agent route missing provider/model'

      const persona = personaFor(parsed, model)
        + `\n\nCurrent routing identity: mode=${bandFor(parsed)}, provider=${provider}, model=${model}.`
      const maxTokens = Number(args.maxTokens || 1024)
      try {
        const run = await ctx.subagents.start('spawn', {
          label: `mode-subagent ${bandFor(parsed)}`,
        ...
        if (result.stopReason !== 'completed') {
          ...
          return `subagent ${result.stopReason}: ${diagnostic}\n${outputText.slice(0, 3000)}`
        }
        await run.dispose()
        const head = outputText.slice(0, 3000)
        return `[mode-subagent ${bandFor(parsed)}]\n${head}${outputText.length > 3000 ? '\n…(truncated)' : ''}`
      } catch (error) {
